...
`.github/workflows/collect-candidates.yml` は6時間ごと、および手動実行時に候補を取り込みます。収集で保存するのは見出し、URL、配信日と管理用メタデータだけで、記事本文や画像は自動取得しません。候補から記事入力を作成して公開状態へ進めるには、引き続き事実確認と編集レビューが必要です。
